...
method_vector_train <- c("", "pmm", "", "", "pmm",
                   "pmm", "pmm", "pmm", "", "pmm",
                   "pmm", "pmm", "", "", "polr", "")
mice_imp_train = mice(input_training, m = 5, method = method_vector_train)
clean_wine_training_data <- complete(mice_imp_train, 3)

input_evaluation <- wine_evaluation_data

# Let mice determine default method structure
init_eval <- mice(input_evaluation, maxit = 0)
method_vector_eval <- init_eval$method

# Override only the columns that need imputation
method_vector_eval["ResidualSugar"] <- "pmm"
method_vector_eval["Chlorides"] <- "pmm"
method_vector_eval["FreeSulfurDioxide"] <- "pmm"
method_vector_eval["pH"] <- "pmm"
method_vector_eval["Sulphates"] <- "pmm"
method_vector_eval["Alcohol"] <- "pmm"
method_vector_eval["STARS"] <- "polr"

# Perform imputation
mice_imp_eval <- mice(input_evaluation, m = 5, method = method_vector_eval)
clean_wine_evaluation_data <- complete(mice_imp_eval, 3)
...
final_training_data <- clean_wine_training_data %>%
  dplyr::select(
    TARGET, STARS, LabelAppeal, Alcohol, TotalSulfurDioxide, 
    FreeSulfurDioxide, ResidualSugar, pH, AcidIndex,
    TotalAcidity, AcidityRatio, SulfurToAcidRatio, 
    AlcoholToSugarRatio, pHToAcidityRatio
  )
```
...
```{r}
enhanced_poisson <- glm(TARGET ~ STARS + LabelAppeal + 
                        log(Alcohol + 1) + log(TotalSulfurDioxide + 1) + 
                        log(FreeSulfurDioxide + 1) + log(ResidualSugar + 1) + 
                        TotalAcidity + AcidityRatio + 
                        SulfurToAcidRatio + AlcoholToSugarRatio + 
                        pHToAcidityRatio + AcidIndex,
                      data = final_training_data,
...
final_evaluation_data <- clean_wine_evaluation_data %>%
  mutate(
    ...
    TotalAcidity = FixedAcidity + VolatileAcidity + CitricAcid,
    AcidityRatio = ifelse(VolatileAcidity > 0, FixedAcidity / VolatileAcidity, NA),
    SulfurToAcidRatio = TotalSulfurDioxide / (FixedAcidity + VolatileAcidity + CitricAcid),
    AlcoholToSugarRatio = ifelse(ResidualSugar > 0, Alcohol / ResidualSugar, NA),
    pHToAcidityRatio = pH / (FixedAcidity + VolatileAcidity + CitricAcid)
  )
