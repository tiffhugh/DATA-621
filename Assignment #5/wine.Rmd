...
For the continuous variables, we will use Predictive Mean Matching (PMM) from the mice package, as it preserves the original data distribution and handles non-normality and skewness effectively. For the ordinal variable `STARS`, we will use the polr method, which is appropriate for ordered categorical data under the proportional odds assumption. We will set the number of imputations (m = 5), a common and computationally efficient choice for large datasets, and run the algorithm for a maximum of 10 iterations (maxit = 10) to ensure convergence while capturing variability in the imputed values.
...
mice_imp_train <- mice(wine_training_data, m = 5, 
                       method = method_vector_train)
summary(wine_training_data$FixedAcidity)
...
mice_imp_train <- mice(wine_evaluation_data, m = 5, 
                       method = method_vector_train)
summary(wine_evaluation_data$FixedAcidity)
mice_imp_train$imp$FixedAcidity
wine_evaluation_data <- complete(mice_imp_train, 3)

mice_imp_train <- mice(wine_training_data, m = 5, method = method_vector_train)
