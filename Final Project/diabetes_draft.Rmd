...
model1 <- gam(target ~ s(bmi) + s(fasting_blood_glucose) + s(cholesterol_total) +
               +s(ggt) + s(serum_urate) + s(dietary_intake_calories)
...
```{r, echo=FALSE}
model2 <- gam(target ~ s(bmi) + s(fasting_blood_glucose) + s(cholesterol_total) +
                s(dietary_intake_calories) + physical_activity_level + 
                alcohol_consumption + smoking_status + ethnicity + s(age) + sex +
                family_history_of_diabetes + s(blood_pressure_systolic) +
                s(blood_pressure_diastolic) + s(cholesterol_hdl) +
                s(cholesterol_ldl) + s(ggt) + s(serum_urate) +
...
```{r, echo=FALSE}
model3 <- gam(target ~ s(fasting_blood_glucose) + s(cholesterol_total) +
                s(cholesterol_hdl), 
              data = diabetes_training_data)
summary(model3)
plot(model3)
...
rf_model <- randomForest(
  target ~ age + sex + ethnicity + bmi + waist_circumference +
    fasting_blood_glucose + blood_pressure_systolic + blood_pressure_diastolic +
...
# View numeric variable importance
importance(rf_model)
