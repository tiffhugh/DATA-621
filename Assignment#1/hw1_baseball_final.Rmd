...
#For our moneyball training data set we have 2276 observations and 17 columns. 
...
# we introduced a new variable called HR_efficiency
moneyball_training_data$HR_Efficiency <- moneyball_training_data$TEAM_BATTING_HR / moneyball_training_data$TEAM_BATTING_H
...
# we simplify the model by removing variables that are not statistically significant or are redundant.
model2 <- lm(TARGET_WINS ~ TEAM_BATTING_HR + TEAM_BASERUN_SB +
              TEAM_FIELDING_E + TEAM_PITCHING_SO + TEAM_PITCHING_H,
              data = moneyball_training_data)
summary(model2)
...
# Finally, we experiment with transformations and interactions.
model3 <- lm(TARGET_WINS ~ log(TEAM_BASERUN_SB + 1) + TEAM_FIELDING_E +
              HR_Efficiency + TEAM_PITCHING_SO + TEAM_BATTING_HR * TEAM_PITCHING_SO,
              data = moneyball_training_data)
summary(model3)
...
# compare the Adjusted R-squared values for all three models.
summary(model1)$adj.r.squared
summary(model2)$adj.r.squared
summary(model3)$adj.r.squared
```
