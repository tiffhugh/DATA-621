...
one dollar increase in vehicle value, the predicted claim increases by $0.11, which is 
reasonable since more expensive cars tend to result in more expensive claims.

- Based on gender, male drivers, on average, have $632 higher claims than female
...
- Surprisingly, drivers with revoked licenses have $708 lower claim amounts.
...
- Married individuals tend to have $549.74 lower claims compared to non-married drivers. 
...
- For every additional year of car age, the predicted claim amount decreases by $51.64. 
...
#| fig-width: 10
# MLR
cat("Summary of Manually Chosen Multiple Linear Regression Model (Model 4): \n")
summary(mlr_model_1)
sqrt(mean(residuals(mlr_model_1)^2))  # RMSE

# Stepwise
cat("Summary of Stepwise Selected Logistic Regression Model (Model 5): \n")
summary(stepwise_lm)
sqrt(mean(residuals(stepwise_lm)^2))  # RMSE

# check linearity, homoscedasticity, outliers
par(mfrow = c(2, 2))
cat("Plot of Residuals for Manually Chosen Multiple Linear Regression Model (Model 4): \n")
plot(mlr_model_1)
cat("Plot of Residuals for Stepwise Selected Logistic Regression Model (Model 5): \n")
plot(stepwise_lm)
```
...
The model had an adjusted R² of 0.012 and an RMSE of 7661, suggesting a poor fit
...
better adjusted R² of 0.017 and a comparable RMSE of 7666. While most predictors
were only marginally significant, the overall model was statistically stronger 
(F-statistic = 8.499, p-value = 5.592e-08) and easier to interpret.
...
prediction_class_eval_model1 <- ifelse(prediction_prob_eval_model1 > 
                                         threshold, 1, 0)
head(prediction_class_eval_model1, 20)
```
...
prediction_class_eval_model2 <- ifelse(prediction_prob_eval_model2 > 
                                         threshold, 1, 0)
head(prediction_class_eval_model2, 20)
```
...
prediction_class_eval_model3 <- ifelse(prediction_prob_eval_model3 > 
                                         threshold, 1, 0)
head(prediction_class_eval_model3, 20)
```
