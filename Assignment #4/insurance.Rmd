...
The `step` function selects a model by different criteria in a stepwise algorithm and helps refining our logistic regression model - and here's what happened:
```{r}
step_model <- step(logit_model, direction = "both")
summary(step_model)

```
We used stepwise selection to simplify the model by removing unnecessary variables. It removed the least useful variable (RED_CAR). Dropping it slightly improved the model's AIC (from 7910.1 to 7908.6). 

The final model dropped RED_CAR, as it had little predictive value and didn’t improve model performance. The updated model includes key predictors like age, income, marital status, car use, education, claim frequency, and motor vehicle record points. The AIC dropped slightly, indicating a better balance between complexity and fit.

### Filter Data to Only Include People Who Crashed

Since TARGET_AMT is only meaningful when someone actually had a crash (i.e., TARGET_FLAG == 1), we’ll model on just those rows