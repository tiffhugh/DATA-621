...
table(Predicted = logit_class, Actual = clean_insurance_training_data$TARGET_FLAG)

# accuracy
mean(logit_class == clean_insurance_training_data$TARGET_FLAG)
```
- 5699 people correctly predicted not to crash (true negatives)
- 485 correctly predicted to crash (true positives)
- 1668 false negatives — people who did crash, but we predicted they wouldn’t
- 309 false positives — predicted to crash, but they didn’t

The model achieves 75.7% accuracy, which is solid, especially considering the class imbalance. Since crashes only occur in about 22% of cases, the model is naturally skewed toward predicting non-crashes, which isn’t necessarily a problem in the context of an insurance company.
...
majority <- clean_insurance_training_data %>% filter(TARGET_FLAG == 0)
minority <- clean_insurance_training_data %>% filter(TARGET_FLAG == 1)
...
# get predicted probabilities
logit_preds <- predict(logit_model, newdata = balanced_data, type = "response")

# convert to 0/1 predictions using 0.5 threshold
logit_class <- ifelse(logit_preds > 0.5, 1, 0)

# the confusion matrix
table(Predicted = logit_class, Actual = balanced_data$TARGET_FLAG)

# accuracy
mean(logit_class == balanced_data$TARGET_FLAG)
```

After undersampling, the model's accuracy dropped from 75.7% to 66.7%, but it correctly identified far more actual crashes (1,423 vs. 485 true positives). If our goal is to improve crash detection — which is important in an insurance setting — the undersampled model is the better choice, especially since we're keeping the prediction threshold fixed at 0.5.

### Stepwise Selected Logistic Regression Model

In insurance risk assessment, predicting crash probability is critical for determining premiums and identifying high-risk factors. Stepwise regression is a statistical method that iteratively selects the most significant predictors (e.g., driver age, vehicle type, driving history) by evaluating criteria such as p-values or Akaike Information Criterion (AIC). This approach strikes a balance between model simplicity and predictive accuracy, ensuring that only variables with a substantial impact on crash likelihood are included. We will perform a stepwise selected logistic regression model to compare the results with the chosen manual logistic regression model for crash probability.
